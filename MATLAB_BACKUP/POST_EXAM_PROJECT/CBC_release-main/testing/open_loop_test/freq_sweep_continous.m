...
amp_val = 10;        % constant forcing amplitude
...
f_num = 50;
sweeps(1).name = "up";
sweeps(1).frequencies = linspace(f_min, f_max, f_num);
sweeps(2).name = "down";
sweeps(2).frequencies = linspace(f_max, f_min, f_num);
...
for sweep_idx = 1:2
    ...
    save("saves/OL_blade_" + sweep.name + "/blade_amp_" + amp_str + ".mat", "data_out");
