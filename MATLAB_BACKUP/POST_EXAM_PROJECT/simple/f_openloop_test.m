function [trial] = f_openloop_test(model, input, trial)
    ...
    [t_lin, x_lin] = f_get_last_n_periods(trial.t, trial.X(:,1), num_periods);
    
    harmonics = 7;%number of harmonics of interest
    [A_vec,B_vec] = f_get_fft_components(t_lin, x_lin, input.w0, trial.harmonics);

    trial.ss_response_amp = sqrt(sum(A_vec.^2 + B_vec.^2, "all"));
    % trial.ss_response_amp = max(x_lin);
